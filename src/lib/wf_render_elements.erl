...
-spec render_and_trap_actions(Elements :: body() | fun()) ->
	{ok, Html :: binary(), Actions :: binary()}.
...
inner_render_elements(Atom) when is_atom(Atom) ->
	wf:to_binary(Atom);
inner_render_elements(E) when
	is_integer(E); is_binary(E); ?IS_STRING(E)
->
	E;
...
inner_render_element(Base = #elementbase{show_if = true}, Element) ->
	...
	case code:ensure_loaded(Module) of
		...
		Response ->
			...
			exit(
				{failed_to_ensure_module_loaded, [
					{element, ElementName},
					{'response_from code:ensure_loaded', Response},
					{module, Module},
					{record, Element}
				]}
			)
...
-spec prepare_and_render_or_transform(Module :: atom(), #elementbase{}, nitrogen_element()) ->
	html().
prepare_and_render_or_transform(Module, Base, Element) ->
...
% HTML.
-spec call_element_render(
	RenderOrTransform :: render_element | transform_element,
	Module :: module(),
...
-spec temp_id() -> string().
temp_id() ->
	%% For Erlang 18+, is erlang:unique_integer,
	Num = ?WF_UNIQUE,
	...
	try
		...
		E:T ->
			error_logger:warning_msg(
				"Error trying to apply function to an element tuple: ~p~nError: ~p: ~p.~nStacktrace: ~p",
				[Rec0, E, T, erlang:get_stacktrace()]
			),
			Rec0
	end;
recurse_body(_Fun, X) ->
