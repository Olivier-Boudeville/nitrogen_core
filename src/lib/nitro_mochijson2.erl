...
parse_decoder_options([{format, Format} | Rest], State) when
    Format =:= struct orelse Format =:= eep18 orelse Format =:= proplist
->
...
input_validation_test() ->
    Good = [
        %% pound
        {16#00A3, <<?Q, 16#C2, 16#A3, ?Q>>},
        %% euro
        {16#20AC, <<?Q, 16#E2, 16#82, 16#AC, ?Q>>},
        %% denarius
        {16#10196, <<?Q, 16#F0, 16#90, 16#86, 16#96, ?Q>>}
    ],
    lists:foreach(
        fun({CodePoint, UTF8}) ->
...
inline_json_test() ->
    ...
    ?assertEqual(
        {struct, [{<<"key">>, <<"iodata iodata">>}]},
        decode(
            encode({struct, [{key, {json, [<<"\"iodata">>, " iodata\""]}}]})
        )
    ),
    ok.
...
atom_test() ->
    ...
    [
        begin
            ...
            ?assertEqual(
                iolist_to_binary(atom_to_list(A)),
...
large_int_test() ->
    ?assertEqual(
        <<"-2147483649214748364921474836492147483649">>,
