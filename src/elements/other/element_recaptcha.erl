...
render_element(
    ...
    %wf:temp_id(),
    ChallengeFieldID = "recaptcha_challenge_field",
...
event({eval_recaptcha, Rec, ChallengeFieldID, ResponseFieldID, FailPanelID}) ->
    ...
    case Evaled of
        ...
        {error, Error} -> process_failure_callback(FailPanelID, FailBody, Tag, Delegate, Error)
    end.
...
challenge_url(#recaptcha{challenge_url = Url}) ->
    ?WF_IF(
        Url =/= undefined, Url, wf:coalesce([get_config(challenge_url), ?DEFAULT_CHALLENGE_URL])
    ).
