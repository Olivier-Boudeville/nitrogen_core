...
render_element(Record) ->
    ...
    PostbackInfo = wf_event:serialize_event_context(
        {Delegate, Tag}, Anchor, undefined, false, ?MODULE
    ),
    Handle =
        case Record#sortblock.handle of
            undefined -> "null";
            Other -> wf:f("'.~s'", [Other])
        end,
    ...
    Script = #script{
        script = wf:f(
            "Nitrogen.$sortblock('~s', {handle: ~s, connectWith: [~s], placeholder: '~s', forcePlaceholderSize: ~s}, '~s');",
            [Anchor, Handle, ConnectWithGroups, Placeholder, ForcePlaceholderSize, PostbackInfo]
        )
    },
    wf:wire(Script),
