...
render_element(Record) ->
    ...
    % Title...
    Title =
        case Record#google_chart.title of
            undefined -> [];
            [] -> [];
    ...
    TitleStyle = wf:f("&chts=~s,~b", [
        wf:to_list(Record#google_chart.color), Record#google_chart.font_size
    ]),
    ...
    % Axes...
    Axes =
        case Record#google_chart.axes of
            undefined ->
                [];
            [] ->
                [];
            AxesRecords ->
                ProcessedAxes = [
                    process_axis(N - 1, lists:nth(N, AxesRecords))
                 || N <- lists:seq(1, length(AxesRecords))
                ],
    ...
    Data =
        case Record#google_chart.data of
            ...
            DataRecords ->
                ProcessedData = [
                    process_data(N - 1, lists:nth(N, DataRecords))
                 || N <- lists:seq(1, length(DataRecords))
                ],
                DataColors =
                    "&chco=" ++ string:join([X || [X, _, _, _, _, _] <- ProcessedData], ","),
                DataLegends =
                    "&chdl=" ++ string:join([X || [_, X, _, _, _, _] <- ProcessedData], "|"),
                DataScales =
                    "&chds=" ++ string:join([X || [_, _, X, _, _, _] <- ProcessedData], ","),
                DataStyles =
                    "&chls=" ++ string:join([X || [_, _, _, X, _, _] <- ProcessedData], "|"),
                DataValues =
                    "&chd=t:" ++ string:join([X || [_, _, _, _, X, _] <- ProcessedData], "|"),
                MaxValueLength = lists:max([X || [_, _, _, _, _, X] <- ProcessedData]),
                DataLegends1 =
                    case string:strip(DataLegends, both, $|) of
                        "&chdl=" -> [];
                        _ -> DataLegends
                    end,
    ...
    BarSize =
        case MaxValueLength of
            ...
            _ ->
                ...
                IndividualBarSize =
                    (AvailablePixels - GroupSpacerPixels - BarSpacerPixels) /
                        (DataGroupsLength * MaxValueLength),
...
process_axis(N, Axis) ->
    Position =
        case Axis#chart_axis.position of
            top -> "t";
            right -> "r";
            bottom -> "x";
            left -> "y"
...
process_data(_N, Data) ->
    ...
    Styles = wf:f("~b,~b,~b", [
        Data#chart_data.line_width, Data#chart_data.line_length, Data#chart_data.blank_length
    ]),
