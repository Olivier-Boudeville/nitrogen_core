...
render_element(
	Record = #wizard{
		tag = Tag, next_class = NextClass, back_class = BackClass, finish_class = FinishClass
	}
) ->
	% Set up steps...
	...
	wf:assert(
		Record#wizard.titles == undefined orelse
			length(Record#wizard.titles) == length(Record#wizard.steps),
		wizard_incorrect_number_of_titles
	),
	...
	F = fun(N) ->
		...
		ButtonRow = fun(TopOrBot) ->
			#panel{
				class = [wizard_buttons, wf:to_list([TopOrBot, wizard_buttons])],
				body = [
					#singlerow{
						cells = [
							#tablecell{
								class = wizard_buttons_back,
								body = [
									#button{
