...
render_element(Record) ->
				...
				actions = [
					#event{
						...
						actions = [
							...
							#script{
								script = wf:f("obj('~s').focus(); obj('~s').select();", [
									TextBoxID, TextBoxID
								])
							}
						]
				...
				body = [
					...
					#button{
						...
						click = [
							...
							#script{
								script = wf:f("obj('~s').value=obj('~s').defaultValue;", [
									TextBoxID, TextBoxID
								])
							}
						]
	...
	case StartMode of
		%% do nothing, as we already hide above in the style element
		view ->
			ok;
		edit ->
	...
	wf:wire(OKButtonID, TextBoxID, #validate{
		attach_to = CancelButtonID, validators = Record#inplace_textarea.validators
	}),
