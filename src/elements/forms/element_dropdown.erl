...
create_options(Selected, HtmlEncode, [#option{show_if = false} | Rest]) ->
	create_options(Selected, HtmlEncode, Rest);
create_options(Selected, HtmlEncode, [Other | Rest]) when
	?IS_STRING(Other); is_binary(Other); is_integer(Other); is_atom(Other)
->
	[
		create_option_simple(Selected, HtmlEncode, Other)
		| create_options(Selected, HtmlEncode, Rest)
	];
create_options(_, _, [Other | _]) ->
	throw({unknown_option_provided_to_dropdown_element, Other}).

create_option_group(Selected, HtmlEncode, #option_group{
	text = Text, options = Options, disabled = Disabled
}) ->
...
create_option_full(
	Selected, HtmlEncode, Opt = #option{text = Text, value = Value, disabled = Disabled}
) ->
...
is_selected(_DropDownValue, #option{selected = true}) ->
	...
	true;
is_selected(DropDownValue, #option{selected = OptSelected, value = OptValue}) when
	OptSelected =:= false;
	OptValue =:= undefined;
