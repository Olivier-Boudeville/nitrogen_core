...
->
    ...
    Containers = [
        {body, wf_utils:get_field(body, Fields, New)},
...
set_html_name(Tag, Fields, Rec) when
    ?IS_FORM(Tag)
->
    {ID, Name} = {wf_utils:get_field(id, Fields, Rec), wf_utils:get_field(html_name, Fields, Rec)},
    {_, NewName} = wf_tags:html_name(ID, Name),
    wf_utils:replace_field(
        html_name,
