...
is_time_to_recache(BinFilePath, FileKey) ->
    ...
    LastRecached = wf:cache({template_last_recached, FileKey}, infinity, Never),
    ...
    LastModified = wf:cache(
        {template_last_modified, FileKey},
        1000,
        GetLastModified
    ),
...
parse_template({content, Binary}, FromType, ToType, Callouts, []) when
    FromType =:= ToType
->
    case Callouts of
