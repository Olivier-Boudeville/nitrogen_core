...
init_request(Bridge) ->
	...
	trace_utils:debug_fmt( "Initialising request based on bridge: ~n ~p",
						   [ Bridge ] ),
...
% Returns {ok, NewReq}:
run(Bridge) ->
	trace_utils:debug_fmt( "nitrogen:run/1 with~n Bridge = ~p", [ Bridge ] ),
	init_request(Bridge),
...
ws_info(Msg, _Bridge, _State) ->
	error_logger:warning_msg("Unhandled message(~p) to websocket process (~p)~n",
							 [Msg, self()]),
	noreply.
