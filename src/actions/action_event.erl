...
render_action(#event{
    ...
    PostbackScript = wf_event:generate_postback_script(
        Postback, Anchor, ValidationGroup1, HandleInvalid, OnInvalid, Delegate, ExtraParam
    ),
    SystemPostbackScript = wf_event:generate_system_postback_script(
        Postback, Anchor, ValidationGroup1, HandleInvalid, Delegate
    ),
        ...
        case Type of
            ...
            system ->
                ...
                [
                    ...
                    SystemPostbackScript,
                    WireAction,
                    "};",
                    wf:f(<<"setTimeout(\"document.~s(); document.~s=null;\", ~p);">>, [
                        TempID, TempID, Delay
                    ])
                ];
            %% USER EVENTS %%%
            ...
            _ when
                ((EffectiveType == keypress orelse EffectiveType == keydown orelse
                    EffectiveType == keyup) andalso (EffectiveKeyCode /= undefined))
            ->
                [
                    wf:f(<<"Nitrogen.$observe_event('~s', '~s', '~s', function(event) {">>, [
                        Anchor, Trigger, EffectiveType
                    ]),
                    wf:f(<<"if (Nitrogen.$is_key_code(event, ~p, ~p)) { ">>, [
                        EffectiveKeyCode, ShiftKey
                    ]),
                    AnchorScript,
                    PostbackScript,
                    WireAction,
            ...
            _ ->
                [
                    wf:f(
                        <<"Nitrogen.$observe_event('~s', '~s', '~s', function(event) {setTimeout(function(){ ">>,
                        [Anchor, Trigger, Type]
                    ),
                    AnchorScript,
                    PostbackScript,
...
maybe_wire_next(_Anchor, undefined) ->
    do_nothing;
