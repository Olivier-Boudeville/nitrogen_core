...
render_action(Record) ->
    ...
    [
        ...
        wf:f(<<"var v = new LiveValidation(obj('~ts'), { onlyOnSubmit: true ~s});">>, [
            TargetPath, InsertAfterNode
        ]),
        wf:f(
            <<"v.add(Validate.Custom, { against: Nitrogen.$return_false, failureMessage: \"~ts\", displayMessageWhenEmpty: true });">>,
            [Text]
        ),
        <<"v.validate();">>
    ].
