...
-define(WF_EXTEND(OrigRec, NewRec, Module, Fields),
    -extend({OrigRec, NewRec, [{module, Module} | Fields]})
).
...
-record(event_context, {
    % The module that should handle the event
    module,
    % An Erlang term that is passed along with the event
    tag,
    % The type of event postback, comet, continuation, upload
    type,
    % The element id to which trigger and target are relative.
    anchor,
    % The validation group that should be run when this event is fired.
    validation_group,
    % When an invalidation evaluates to false, instead of silently not
    handle_invalid
...
-record(handler_context, {
    % The name of a handler. See wf_context for a list.
    name,
    % A module that provides the logic for a handler. This can be substituted by your app.
    module,
    % The config of the handler, set at the beginning of each request.
    config,
    % The state of the handler, serialized and maintained between postbacks in a series.
    state
...
-record(context, {
    ...
    type :: context_type(),
    %% will only be undefined when "not in a request"
    bridge :: undefined | simple_bridge:bridge(),
    anchor = undefined :: id(),
    data = [] :: context_data(),
...
-ifndef(LOGGER_HRL).
-define(PRINT(Var),
    error_logger:info_msg("DEBUG: ~p~n~p:~p~n~p~n  ~p~n", [self(), ?MODULE, ?LINE, ??Var, Var])
).
...
-record(function_el, {
    ?ELEMENT_BASE(element_function),
    function = fun() -> [] end :: fun() | [fun()]
}).
-record(body, {
    ?ELEMENT_BASE(element_body),
    body = [] :: body()
}).

-define(H_ELEMENT(Size), {
    ?ELEMENT_BASE(element_h),
    text = "" :: text(),
    body = [] :: body(),
...
-record(list, {
    ?ELEMENT_BASE(element_list),
    numbered = false :: boolean(),
    body = [] :: body()
}).
-record(listitem, {
    ?ELEMENT_BASE(element_listitem),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    role = "" :: term()
...
-record(p, {
    ?ELEMENT_BASE(element_p),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(i, {
    ?ELEMENT_BASE(element_i),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(label, {
    ?ELEMENT_BASE(element_label),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    for = "" :: id()
...
-record(pre, {
    ?ELEMENT_BASE(element_pre),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(strong, {
    ?ELEMENT_BASE(element_strong),
    body = "" :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(em, {
    ?ELEMENT_BASE(element_em),
    body = "" :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(value, {
    ?ELEMENT_BASE(element_value),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(link, {
    ?ELEMENT_BASE(element_link),
    text = "" :: text(),
    body = "" :: body(),
...
-record(email_link, {
    ?ELEMENT_BASE(element_email_link),
    text = "" :: text(),
    body = "" :: body(),
    html_encode = true :: html_encode(),
    email = "" :: text()
...
-record(error, {
    ?ELEMENT_BASE(element_error),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(span, {
    ?ELEMENT_BASE(element_span),
    body = "" :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(delay_body, {
    ?ELEMENT_BASE(element_delay_body),
    delegate :: module(),
    tag = undefined :: term(),
    placeholder :: undefined | body(),
    method = optimized :: optimized | simple,
    %% milliseconds to wait to populate
    delay = 0 :: integer()
}).
-record(button, {
    ?ELEMENT_BASE(element_button),
    text = "" :: text(),
    body = "" :: body(),
    image = undefined :: undefined | url(),
    html_encode = true :: html_encode(),
...
-record(literal, {
    ?ELEMENT_BASE(element_literal),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(textbox, {
    ?ELEMENT_BASE(element_textbox),
    text = "" :: text(),
...
-record(datepicker_textbox, {
    ?ELEMENT_BASE(element_datepicker_textbox),
    text = "" :: text(),
    maxlength = "" :: integer() | string(),
    size = "" :: integer() | string(),
    placeholder = "" :: text(),
...
-record(date_dropdown, {
    ?ELEMENT_BASE(element_date_dropdown),
...
-record(textbox_autocomplete, {
    ?ELEMENT_BASE(element_textbox_autocomplete),
    tag :: term(),
    text = "" :: text(),
    minLength = 2 :: integer(),
    size = "" :: integer() | string(),
...
-record(radiogroup, {
    ?ELEMENT_BASE(element_radiogroup),
    body = [] :: body()
}).
-record(radio, {
    ?ELEMENT_BASE(element_radio),
    body = "" :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    value :: text(),
...
-record(restful_form, {
    ?ELEMENT_BASE(element_restful_form),
...
-record(restful_submit, {
    ?ELEMENT_BASE(element_restful_submit),
    text = "Submit" :: text(),
    body = [] :: body(),
    html_encode = true :: html_encode(),
    html_name :: html_name()
}).
-record(restful_reset, {
    ?ELEMENT_BASE(element_restful_reset),
    text = "Cancel" :: text(),
    html_encode = true :: html_encode(),
    html_name :: html_name()
}).
-record(restful_upload, {
    ?ELEMENT_BASE(element_restful_upload),
    %        text="Browse"           :: text(),
    %        html_encode=true        :: html_encode(),
    html_name :: html_name()
}).
-record(panel, {
    ?ELEMENT_BASE(element_panel),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode()
}).
-record(sync_panel, {
    ?ELEMENT_BASE(element_sync_panel),
    body_fun :: undefined | fun(),
    triggers = [] :: [term()],
    pool = sync_panel :: atom()
}).
-record(fieldset, {
    ?ELEMENT_BASE(element_fieldset),
    body = [] :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    legend_body = [] :: body(),
...
-record(spinner, {
    ?ELEMENT_BASE(element_spinner),
    image = "/nitrogen/spinner.gif" :: url()
}).
-record(image, {
    ?ELEMENT_BASE(element_image),
    image = "" :: url(),
    alt = "" :: text(),
...
-record(lightbox, {
    ?ELEMENT_BASE(element_lightbox),
    body = [] :: body()
}).
-record(table, {
    ?ELEMENT_BASE(element_table),
    rows :: body(),
    border = 0 :: integer(),
    header = [] :: body(),
    footer = [] :: body()
...
-record(tablerow, {
    ?ELEMENT_BASE(element_tablerow),
    cells :: body()
}).
-record(tableheader, {
    ?ELEMENT_BASE(element_tableheader),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    body = [] :: body(),
...
-record(tablecell, {
    ?ELEMENT_BASE(element_tablecell),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    body = "" :: body(),
    align = "left" :: text() | atom(),
...
-record(singlerow, {
    ?ELEMENT_BASE(element_singlerow),
    cells :: body()
}).
-record(file, {
...
-record(placeholder, {
    ?ELEMENT_BASE(element_placeholder),
    body = [] :: body()
}).
-record(bind, {
    ?ELEMENT_BASE(element_bind),
    data = [] :: list(),
    map = [] :: list() | tuple(),
...
-record(sortblock, {
    ?ELEMENT_BASE(element_sortblock),
    tag :: term(),
    items = [] :: body(),
...
-record(sortitem, {
    ?ELEMENT_BASE(element_sortitem),
    tag :: term(),
    body = [] :: body()
}).
-record(draggable, {
    ?ELEMENT_BASE(element_draggable),
    tag :: term(),
    body = [] :: body(),
    group :: atom() | string() | binary(),
    handle :: class(),
...
-record(droppable, {
    ?ELEMENT_BASE(element_droppable),
    tag :: term(),
    body = [] :: body(),
...
-record(inplace_textarea, {
    ?ELEMENT_BASE(element_inplace_textarea),
    tag :: term(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
...
-record(inplace_textbox, {
    ?ELEMENT_BASE(element_inplace_textbox),
    tag :: term(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
    start_mode = view :: view | edit,
...
-record(mobile_list, {
    ?ELEMENT_BASE(element_mobile_list),
    body = [] :: body(),
    theme :: mobile_theme(),
    inset = true :: boolean()
}).
-record(mobile_list_divider, {
    ?ELEMENT_BASE(element_mobile_list_divider),
    theme :: mobile_theme(),
    text = "" :: text(),
    body = [] :: body(),
    role = heading :: atom() | string()
}).
-record(mobile_listitem, {
    ?ELEMENT_BASE(element_mobile_listitem),
    text = "" :: text(),
    body = [] :: body(),
    theme = "" :: atom() | string()
}).
-record(mobile_toggle, {
    ?ELEMENT_BASE(element_mobile_toggle),
    on_text = "On" :: text(),
    on_value = "on" :: text(),
    off_text = "Off" :: text(),
    off_value = "off" :: text(),
...
-record(mobile_collapsible, {
    ?ELEMENT_BASE(element_mobile_collapsible),
...
-record(mobile_collapsible_set, {
    ?ELEMENT_BASE(element_mobile_collapsible_set),
    header_theme :: mobile_theme(),
    content_theme :: mobile_theme(),
    mini = false :: boolean(),
    body = [] :: body()
}).
-record(mobile_grid_block, {
    ?ELEMENT_BASE(element_mobile_grid_block),
    text = "" :: text(),
    body = [] :: body(),
    new_row = default :: atom()
}).
-record(mobile_grid, {
    ?ELEMENT_BASE(element_mobile_grid),
    columns = 2 :: integer(),
    blocks = [] :: [#mobile_grid_block{}]
}).
-record(mobile_panel, {
    ?ELEMENT_BASE(element_mobile_panel),
    mini = false :: boolean(),
...
-record(section, {
    ?ELEMENT_BASE(element_section),
    body = "" :: body(),
    role = "" :: text()
}).
-record(nav, {
    ?ELEMENT_BASE(element_nav),
    body = "" :: body(),
    role = "" :: text()
}).
-record(article, {
    ?ELEMENT_BASE(element_article),
    body = "" :: body(),
    role = "" :: text()
}).
-record(aside, {
    ?ELEMENT_BASE(element_aside),
    body = "" :: body(),
    role = "" :: text()
}).
-record(html5_header, {
    ?ELEMENT_BASE(element_html5_header),
    body = "" :: body(),
    role = "" :: text()
}).
-record(html5_footer, {
    ?ELEMENT_BASE(element_html5_footer),
    body = "" :: body(),
    role = "" :: text()
}).
-record(time, {
    ?ELEMENT_BASE(element_time),
    datetime = "" :: text(),
    body = "" :: body(),
    text = "" :: text(),
    html_encode = true :: html_encode(),
...
-define(GRID_ELEMENT(Type, Columns), {
    ?ELEMENT_BASE(element_grid),
...
-record(progress_bar, {
    ?ELEMENT_BASE(element_progress_bar),
...
-record(google_chart, {
    ?ELEMENT_BASE(element_google_chart),
...
-record(qr, {
    ?ELEMENT_BASE(element_qr),
    data = undefined :: any(),
    size = 200 :: integer()
}).

-record(mermaid, {
    ?ELEMENT_BASE(element_mermaid),
    code = [] :: mermaid_code(),
    options = [] :: proplist(),
...
-define(ACTION_UPDATE(Type), {
    ?ACTION_BASE(action_update), type = Type :: atom(), elements = [] :: body()
}).
...
-record(api, {
    ?ACTION_BASE(action_api),
    name :: string() | binary() | atom(),
    tag :: term(),
    delegate :: module()
}).
-record(function, {
    ?ACTION_BASE(action_function),
    function :: fun() | undefined
}).
-record(js_fun, {
    ?ACTION_BASE(action_js_fun),
    function :: atom() | text(),
    args = [] :: [text()]
}).
-record(set, {
    ?ACTION_BASE(action_set),
    value = "" :: text() | integer() | atom()
}).
-record(set_multiple, {
    ?ACTION_BASE(action_set_multiple),
    values = [] :: [text()]
}).
-record(redirect, {
    ?ACTION_BASE(action_redirect),
    url = "" :: url(),
    login = false :: boolean() | url()
}).
-record(open_window, {
    ?ACTION_BASE(action_open_window),
    url = "" :: url(),
...
-record(validation_error, {
    ?ACTION_BASE(action_validation_error),
    text = "" :: text(),
    attach_to :: id()
}).
-record(clear_validation, {
    ?ACTION_BASE(action_clear_validation),
    validation_trigger :: id(),
    validation_target :: id(),
    validation_all :: id()
}).
-record(alert, {
    ?ACTION_BASE(action_alert),
    text = "" :: text()
}).
-record(confirm, {
    ?ACTION_BASE(action_confirm),
    text = "" :: text(),
    postback :: term(),
    delegate :: module()
}).
-record(console_log, {
    ?ACTION_BASE(action_console_log),
    text = "" :: any()
}).
-record(script, {
    ?ACTION_BASE(action_script),
    script :: text()
}).
-record(disable_selection, {?ACTION_BASE(action_disable_selection)}).
-record(disable_option, {
    ?ACTION_BASE(action_toggle_option),
    value :: atom() | string() | binary() | integer()
}).
-record(enable_option, {
    ?ACTION_BASE(action_toggle_option),
    value :: atom() | string() | binary() | integer()
}).
-record(add_option, {
    ?ACTION_BASE(action_add_option),
    option :: #option{} | short_option(),
    location = bottom :: top | bottom
}).
-record(remove_option, {
    ?ACTION_BASE(action_remove_option),
    value :: atom() | string() | binary() | integer()
}).
-record(jquery_effect, {
    ?ACTION_BASE(action_jquery_effect),
...
-record(show, {
    ?ACTION_BASE(action_show),
    effect = none :: atom(),
    options = [] :: proplist(),
    speed = 500 :: integer()
}).
-record(hide, {
    ?ACTION_BASE(action_hide),
    effect = none :: atom(),
    options = [] :: proplist(),
    speed = 500 :: integer()
}).
-record(appear, {
    ?ACTION_BASE(action_appear),
    speed = 500 :: integer()
}).
-record(fade, {
    ?ACTION_BASE(action_fade),
    speed = 500 :: integer()
}).
-record(slide_down, {
    ?ACTION_BASE(action_slide_down),
    speed = 500 :: integer()
}).
-record(slide_up, {
    ?ACTION_BASE(action_slide_up),
    speed = 500 :: integer()
}).
-record(effect, {
    ?ACTION_BASE(action_effect),
    effect = none :: atom(),
    options = [] :: proplist(),
    speed = 500 :: integer()
}).
-record(toggle, {
    ?ACTION_BASE(action_toggle),
    effect = none :: atom(),
    options = [] :: proplist(),
    speed = 500 :: integer()
}).
...
-record(add_class, {
    ?ACTION_BASE(action_add_class),
    class = none :: class(),
    speed = 0 :: integer()
}).
-record(remove_class, {
    ?ACTION_BASE(action_remove_class),
    class = none :: class(),
    speed = 0 :: integer()
}).
-record(animate, {
    ?ACTION_BASE(action_animate),
    options = [] :: proplist(),
    speed = 500 :: integer(),
    easing = swing :: atom()
}).
...
-define(VALIDATOR_BASE(Module),
    ?AV_BASE(Module, is_action),
    text = "Failed." :: text(),
    attach_to :: undefined | id()
).
-record(validatorbase, {?VALIDATOR_BASE(undefined)}).
-record(is_required, {
    ?VALIDATOR_BASE(validator_is_required),
    unless_has_value :: undefined | id() | [id()]
}).
-record(is_email, {?VALIDATOR_BASE(validator_is_email)}).
-record(is_integer, {
    ?VALIDATOR_BASE(validator_is_integer),
    min :: undefined | integer(),
    max :: undefined | integer(),
    allow_blank = false :: boolean()
}).
-record(is_number, {
    ?VALIDATOR_BASE(validator_is_number),
    min :: undefined | integer(),
    max :: undefined | integer(),
    allow_blank = false :: boolean()
}).
-record(min_length, {
    ?VALIDATOR_BASE(validator_min_length),
    length :: undefined | integer()
}).
-record(max_length, {
    ?VALIDATOR_BASE(validator_max_length),
    length :: undefined | integer()
}).
-record(confirm_password, {
    ?VALIDATOR_BASE(validator_confirm_password),
    password :: id()
}).
-record(confirm_same, {
    ?VALIDATOR_BASE(validator_confirm_same),
    confirm_id :: id()
}).
-record(custom, {
    ?VALIDATOR_BASE(validator_custom),
    function :: fun(),
    tag :: term()
}).
-record(js_custom, {
    ?VALIDATOR_BASE(validator_js_custom),
    function :: atom() | script(),
    args = "{}" :: text(),
    when_empty = false :: boolean()
}).
-record(if_value, {
    ?ACTION_BASE(action_if_value),
    value :: atom() | text(),
